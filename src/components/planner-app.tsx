...
import type { BikeProfile, Locale, PlannedRoute, RouteMode, Waypoint } from '@/lib/domain'
...
export function PlannerApp() {
  ...
  async function requestRoute(points: Waypoint[]) {
    ...
    try {
      const response = await fetch('/api/routes/plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile, mode, targetDistanceKm: distance, waypoints: points }) })
  ...
  return <main className="app-shell">
    ...
    <aside className={`planner-panel ${mobilePanel ? 'is-open' : ''}`} aria-label="Routeneinstellungen">
      <div className="panel-intro"><p className="eyebrow">{copy.preview}</p><h1>{locale === 'de' ? 'Wohin möchtest du fahren?' : 'Where do you want to ride?'}</h1><p>{locale === 'de' ? 'Setze deine Punkte direkt auf der Karte. Den Rest übernimmt Velvetia.' : 'Choose points on the map. Velvetia takes care of the rest.'}</p></div>
      ...
      <button className="advanced-button">{locale === 'de' ? 'Erweiterte Wünsche' : 'Advanced preferences'} <ChevronDown size={17} /></button>
    </aside>

    {route && <section className="route-summary" aria-label="Routenzusammenfassung">
      <div className="route-summary-head"><div><p className="eyebrow">{copy.routeReady}</p><h2>{route.name}</h2></div><span className="preview-badge">{copy.preview}</span></div>
      ...
      <p className="preview-note">{copy.previewInfo}</p>
