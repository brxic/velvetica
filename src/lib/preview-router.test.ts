...
describe('preview route planner', () => {
  it('creates a closed round trip with usable metrics', () => {
    const route = planPreviewRoute({ profile: 'road', mode: 'round-trip', targetDistanceKm: 60, waypoints: [start] })
  ...
  it('uses an explicit destination for one-way routes', () => {
    ...
    const route = planPreviewRoute({ profile: 'touring', mode: 'one-way', targetDistanceKm: 100, waypoints: [start, end] })
  ...
  it('rejects requests without a starting point', () => {
    expect(() => planPreviewRoute({ profile: 'city', mode: 'round-trip', targetDistanceKm: 20, waypoints: [] })).toThrow('NO_START')
  })
})
